...
1. pondContainsCards(deck) => true/false  
   Checks if the pond/deck still contains cards by checking length of card array and returns true/false
1. goFish(player, deck) => player, deck  
   Requires pond to have cards. Removes one card from the deck and adds it to the players hand. Returns the updated player/deck
...
1. checkForSet(player) => player  
   Checks for sets of four cards of the same value in the player's hand. Removes sets and adds 1 to player's set count.
